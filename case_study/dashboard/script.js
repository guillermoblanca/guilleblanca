...
const industryData = {
    ecommerce: {
        kpi1: { value: '€48.2K', label: 'Revenue' },
        kpi2: { value: '3.4%', label: 'Conversion' },
        kpi3: { value: '€89', label: 'Avg Order' },
        kpi4: { value: '68%', label: 'Cart Abandon' },
        insight: {
    ...
    saas: {
        kpi1: { value: '€22.6K', label: 'MRR' },
        kpi2: { value: '2.1%', label: 'Churn' },
        kpi3: { value: '€-476', label: 'Churned MRR' },
        kpi4: { value: '4.2x', label: 'LTV/CAC' },
        insight: {
    ...
    ops: {
        kpi1: { value: '94%', label: 'On-time' },
        kpi2: { value: '6.2h', label: 'Avg Processing' },
        kpi3: { value: '4.2%', label: 'Returns' },
        kpi4: { value: '47', label: 'Backlog Orders' },
        insight: {
...
function switchIndustry(industry) {
    ...
    // Update KPIs
    document.querySelector('[data-metric="kpi1"]').textContent = data.kpi1.value;
    document.querySelector('[data-metric-label="kpi1"]').textContent = data.kpi1.label;
    document.querySelector('[data-metric="kpi2"]').textContent = data.kpi2.value;
    document.querySelector('[data-metric-label="kpi2"]').textContent = data.kpi2.label;
    document.querySelector('[data-metric="kpi3"]').textContent = data.kpi3.value;
    document.querySelector('[data-metric-label="kpi3"]').textContent = data.kpi3.label;
    document.querySelector('[data-metric="kpi4"]').textContent = data.kpi4.value;
    document.querySelector('[data-metric-label="kpi4"]').textContent = data.kpi4.label;
...
function applyLanguage(lang) {
    ...
    // Update insight if it's been set (avoid overwriting on init)
    ...
    if (insightEl && insightEl.innerHTML) {
        ...
        insightEl.innerHTML = industryData[currentIndustry].insight[lang];
    }
...
// TOC scroll spy
function updateTOC() {
    const sections = ['dashboard', 'how-it-works', 'insight'];
